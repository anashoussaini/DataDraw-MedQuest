...
def upload_json_to_drive(json_data, filename):
    """Uploads a JSON string to a specified Google Drive folder."""
    drive_service = get_gdrive_service()
    folder_id = st.secrets["gcp_drive"]["folder_id"]  # Your target folder ID
    media = MediaInMemoryUpload(json_data.encode('utf-8'), mimetype='application/json')
    file_metadata = {
        'name': filename,
        'parents': [folder_id]
...
def append_metadata_to_gsheet(exam_data):
    """
    Appends exam metadata plus some extra details to a Google Sheet.
    Adjust the range if your sheet is named differently.
    """
    sheets_service = get_gsheet_service()
    spreadsheet_id = st.secrets["gcp_sheets"]["spreadsheet_id"]
    ...
    new_row = [
        timestamp, unique_id, school, exam_year, exam_month, exam_variable,
        subject_year, semester, topic, q_count, metadata_json_str
    ]
...
# ---------------------------
def show_metadata_form(metadata):
    """Display a form for exam metadata and return user inputs."""
    col1, col2 = st.columns(2)
    with col1:
        all_schools = schools_data["Schools"]
        default_school = metadata.get("school", all_schools[0])
    ...
    with col2:
        ...
        topics = curriculum_data[subject_year][semester]
        default_topic = metadata.get("topic", topics[0])
        if default_topic in topics:
            topic_index = topics.index(default_topic)
        else:
            topic_index = 0
            if default_topic:
                st.warning(f"Previously selected topic '{default_topic}' is not available. Please select a new topic.")
        topic = st.selectbox("Topic", options=topics, index=topic_index)

        exam_variable = st.number_input("Exam Variable", min_value=1, max_value=1000,
                                        value=metadata.get("exam_variable", 1),
                                        step=1, help="Enter a number representing the exam instance.")
    return school, exam_year, exam_month, subject_year, semester, topic, exam_variable
...
def show_create_exam_page():
    ...
    def save_draft():
        with open("draft_exam.json", "w") as f:
            json.dump(st.session_state.exam_data, f, indent=2)
        st.success("Draft saved!")
    if st.button("Save Draft"):
        save_draft()
    if os.path.exists("draft_exam.json"):
    ...
    if 'metadata_submitted' in st.session_state:
        ...
        num_questions = st.number_input("Total Number of Questions", min_value=1,
                                        value=len(st.session_state.exam_data["content"]["questions"]) or 1,
                                        step=1)
        current_length = len(st.session_state.exam_data["content"]["questions"])
        if current_length < num_questions:
            for _ in range(num_questions - current_length):
                st.session_state.exam_data["content"]["questions"].append({
                    "question": "",
                    "options": {},
                    "correct_answers": [],
        ...
        st.write(f"Showing questions {start_index + 1} to {end_index} of {total_questions}")

        # --- Render only the questions for the current page ---
        for i in range(start_index, end_index):
            question = questions[i]
            with st.expander(f"Question {i+1}", expanded=True):
                # Use a form so that all changes for this question are submitted together.
                with st.form(key=f"form_question_{i}"):
                    ...
                    # Image upload
                    uploaded_image = st.file_uploader(f"Upload an image for question #{i+1}",
                                                      type=["png", "jpg", "jpeg"],
                                                      key=f"q{i}_image")
                    if uploaded_image:
                        image_obj = Image.open(uploaded_image)
                    ...
                    correct_answers_input = st.text_input("Correct Answer(s) (e.g., ABC, DE, A)",
                                                          ...
                                                          key=f"q{i}_correct_input")
                    new_correct_answers = [ans for ans in correct_answers_input.upper() if ans in new_options]
                    question["correct_answers"] = new_correct_answers
                    question["isAnswered"] = bool(new_correct_answers)
                    st.write("Parsed Correct Answer(s):", ", ".join(new_correct_answers))
